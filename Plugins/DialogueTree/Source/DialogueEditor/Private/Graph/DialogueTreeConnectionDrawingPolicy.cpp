	...
	)
{
	...
	PinRadius = FDialogueEditorStyle::GetVector(TEXT("PinSize")).Y / 2.f;

	//Flip direction of relevant spline settings for vertical flow 
	SplineSettings.FwdDeltaRangeX = Settings->ForwardSplineVerticalDeltaRange;

	SplineSettings.BwdDeltaRangeX = Settings->BackwardSplineVerticalDeltaRange;

	SplineSettings.FwdDeltaRangeY = Settings->ForwardSplineHorizontalDeltaRange;

	SplineSettings.BwdDeltaRangeY = Settings->BackwardSplineHorizontalDeltaRange;

	SplineSettings.FwdTanScaleX = Settings->ForwardSplineTangentFromVerticalDelta.Y;

	SplineSettings.FwdTanScaleY = Settings->ForwardSplineTangentFromHorizontalDelta.X;

	SplineSettings.BwdTanScaleX = Settings->BackwardSplineTangentFromVerticalDelta.Y;

	SplineSettings.BwdTanScaleY = Settings->BackwardSplineTangentFromHorizontalDelta.X;
}

void FDialogueTreeConnectionDrawingPolicy::DrawPreviewConnector(const FGeometry& PinGeometry, const FVector2D& StartPoint, const FVector2D& EndPoint, UEdGraphPin* Pin)
{
...
void FDialogueTreeConnectionDrawingPolicy::DrawSplineWithArrow(const FVector2D& StartPoint, const FVector2D& EndPoint, const FConnectionParams& Params)
{
	// Draw the spline
	...
	if (ArrowImage != nullptr)
	{
		...
		*/
		FPaintGeometry ArrowGeometry = FPaintGeometry(ArrowPoint, ArrowImage->ImageSize * ZoomFactor, ZoomFactor);
...
void FDialogueTreeConnectionDrawingPolicy::DrawSplineWithArrow(const FGeometry& StartGeom, const FGeometry& EndGeom, const FConnectionParams& Params)
{
...
FVector2D FDialogueTreeConnectionDrawingPolicy::ComputeSplineTangent(const FVector2D& Start, const FVector2D& End) const
{
	//Calculate delta 
...
FVector2D FDialogueTreeConnectionDrawingPolicy::GetPinConnectionPoint(const FGeometry& InPinGeom, const EEdGraphPinDirection InDirection) const
{
	FVector2D PinCenter = FGeometryHelper::CenterOf(InPinGeom);

	if (InDirection == EGPD_Input)
	{
		FVector2D InputPinOffset = FVector2D(0.f, PinRadius + ArrowRadius.X);
...
FDialogueTreeConnectionDrawingPolicy::FSplineShape FDialogueTreeConnectionDrawingPolicy::GetSplineShape(FVector2D DeltaPos) const
{
	//Determine directionality
